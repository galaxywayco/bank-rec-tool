  ...
  const handleGLChange = useCallback((val) => {
    ...
    if (val.trim()) {
      ...
      if (rows.length > 0) {
        ...
        const fields = []
        if (cols.dateCol >= 0) fields.push(`date:col${cols.dateCol + 1}`)
        if (cols.debitCol >= 0) fields.push(`debit:col${cols.debitCol + 1}`)
        if (cols.creditCol >= 0) fields.push(`credit:col${cols.creditCol + 1}`)
        setDetectedGL(fields.length ? fields.join(' · ') : null)
      }
    } else {
      setDetectedGL(null)
    }
